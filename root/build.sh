...
cmake -DCMAKE_INSTALL_PREFIX=${project_root_dir}/third_party/matplotplusplus/
...
rm -rf build install
mkdir -p build && mkdir -p install && cd build
cmake -DCMAKE_INSTALL_PREFIX=${project_root_dir}/third_party/glew/install -
...
rm -rf build install
mkdir -p build && mkdir -p install && cd build
cmake -DCMAKE_INSTALL_PREFIX=${project_root_dir}/third_party/glfw/install -
DCMAKE_BUILD_TYPE=Release ..
cmake --build . -j 4
cmake --install .
fi
